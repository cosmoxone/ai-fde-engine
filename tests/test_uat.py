...
def project_with_documents(client, new_project):
    ...
    for filename, content_type, content in docs:
        ...
        with open(tmp, "rb") as f:
            resp = client.post(f"/api/v1/projects/{new_project}/documents", files={"file": (filename, f, content_type)})
        os.unlink(tmp)
        assert resp.status_code == 200
...
def project_with_research(client, project_with_documents):
    ...
    resp = client.post(
        ...
        json={
            "client_requirements": "构建智能客服系统，实现常见问题自动回答、订单查询、物流跟踪，目标降低70%人工客服成本"
        },
    )
    assert resp.status_code == 200
...
def project_with_benchmark(client, project_with_research):
    """生成Benchmark"""
    resp = client.post(f"/api/v1/projects/{project_with_research}/benchmarks/generate", json={"case_count": 15})
    assert resp.status_code == 200
    return project_with_research
...
class TestUATFDE01NewProjectResearch:
    ...
    def test_step1_create_project(self, client, new_project):
        ...
        data = resp.json()
        assert (
            data.get("project", {}).get("name") == "智能客服系统交付项目" or data.get("name") == "智能客服系统交付项目"
        )
    ...
    def test_step3_fill_client_need(self, client, project_with_documents):
        ...
        resp = client.post(
            f"/api/v1/projects/{project_with_documents}/research/run", json={"client_requirements": "构建智能客服系统"}
        )
        assert resp.status_code == 200
    ...
    def test_step9_requirement_baseline(self, client, project_with_research):
        ...
        data = resp.json()
        assert (
            data.get("project", {}).get("status") is not None
    ...
    def test_end_to_end_research_flow(self, client, project_with_documents):
        ...
        # 触发调研
        resp = client.post(
            f"/api/v1/projects/{project_with_documents}/research/run", json={"client_requirements": "智能客服系统"}
        )
        assert resp.status_code == 200

        # 生成Benchmark
        resp2 = client.post(f"/api/v1/projects/{project_with_documents}/benchmarks/generate", json={"case_count": 10})
...
class TestUATFDE03KnowledgeBase:
    ...
    def test_step1_build_kb(self, client, project_with_documents):
        """步骤1：触发知识库构建"""
        resp = client.post(f"/api/v1/projects/{project_with_documents}/delivery/run", json={"task_type": "build_kb"})
        assert resp.status_code == 200

    def test_step2_kb_stats(self, client, project_with_documents):
        """步骤2：查看知识库统计"""
        client.post(f"/api/v1/projects/{project_with_documents}/delivery/run", json={"task_type": "build_kb"})
    ...
    def test_step3_query_kb(self, client, project_with_documents):
        ...
        client.post(f"/api/v1/projects/{project_with_documents}/delivery/run", json={"task_type": "build_kb"})
        resp = client.get(
            f"/api/v1/projects/{project_with_documents}/memory/search", params={"query": "客服流程", "limit": 5}
        )
        assert resp.status_code == 200
...
class TestUATFDE04CodeGeneration:
    ...
    def test_step1_trigger_code_gen(self, client, project_with_design):
        """步骤1：触发代码生成"""
        resp = client.post(f"/api/v1/projects/{project_with_design}/delivery/run", json={"task_type": "generate_code"})
        assert resp.status_code == 200
    ...
    def test_step3_code_syntax(self, client, project_with_design):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("success") is True or "code" in str(data).lower()
...
class TestUATFDE05NightlyIteration:
    ...
    def test_step1_submit_badcases(self, client, project_with_benchmark):
        ...
        badcases = [
            {
                "input": "用户：我的订单什么时候发货？",
                "actual_output": "无法查询订单信息",
                "expected_output": "您的订单已发货，物流单号SF1234567890",
                "error_type": "knowledge_gap",
                "severity": "high",
            },
            {
                "input": "用户：如何申请退货？",
                "actual_output": "退货流程很复杂",
                "expected_output": "退货流程：1.进入订单详情 2.点击申请退货 3.填写原因 4.提交审核",
                "error_type": "format_error",
                "severity": "medium",
            },
            {
                "input": "用户：你们的客服电话是多少？",
                "actual_output": "400-123-4567",
                "expected_output": "客服热线：400-888-8888（工作日9:00-18:00）",
                "error_type": "hallucination",
                "severity": "low",
            },
        ]
        for bc in badcases:
            resp = client.post(f"/api/v1/projects/{project_with_benchmark}/badcases", json=bc)
            assert resp.status_code == 200
    ...
    def test_end_to_end_iteration_flow(self, client, project_with_benchmark):
        ...
        data = resp.json()
        # 迭代结果应包含关键信息
        assert data.get("success") is True or "iteration" in str(data).lower()
...
class TestUATFDE06MemorySystem:
    ...
    def test_step2_memory_search(self, client, project_with_research):
        """步骤2：按关键词检索项目记忆"""
        resp = client.get(
            f"/api/v1/projects/{project_with_research}/memory/search", params={"query": "客服", "limit": 5}
        )
        assert resp.status_code == 200
    ...
    def test_step4_memory_consolidation(self, client, project_with_research):
        ...
        resp = client.post(f"/api/v1/projects/{project_with_research}/memory/consolidate", json={"badcases": []})
        assert resp.status_code == 200

    def test_step5_cross_project_search(self, client, project_with_research):
        """步骤5：创建新项目，触发跨项目检索"""
        resp2 = client.post(
            "/api/v1/projects", json={"name": "新项目-经验复用测试", "client_name": "新客户", "industry": "新零售"}
        )
        new_pid = resp2.json().get("project_id") or resp2.json().get("id") or resp2.json().get("project", {}).get("id")

        # 新项目检索（可能返回空但不应报错）
        resp = client.get(f"/api/v1/projects/{new_pid}/memory/search", params={"query": "客服系统经验", "limit": 5})
        assert resp.status_code == 200
...
class TestUATPM01ProgressTracking:
    ...
    def test_step4_multi_project_list(self, client, project_with_research):
        ...
        for i in range(2):
            client.post(
                "/api/v1/projects", json={"name": f"多项目测试{i}", "client_name": f"客户{i}", "industry": "测试"}
            )
...
class TestUATCUST01BusinessValidation:
    ...
    def test_step2_parse_accuracy(self, client, project_with_documents):
        ...
        # 通过调研结果间接验证解析准确性
        resp = client.post(
            f"/api/v1/projects/{project_with_documents}/research/run", json={"client_requirements": "验证解析准确性"}
        )
        assert resp.status_code == 200
    ...
    def test_step3_kb_qa_trial(self, client, project_with_documents):
        ...
        client.post(f"/api/v1/projects/{project_with_documents}/delivery/run", json={"task_type": "build_kb"})
        resp = client.get(
            f"/api/v1/projects/{project_with_documents}/memory/search", params={"query": "订单查询流程", "limit": 3}
        )
        assert resp.status_code == 200
    ...
    def test_step4_answer_traceability(self, client, project_with_documents):
        """步骤4：验证答案可解释性"""
        resp = client.get(
            f"/api/v1/projects/{project_with_documents}/memory/search", params={"query": "退换货", "limit": 3}
        )
        assert resp.status_code == 200
    ...
    def test_step6_feedback_status(self, client, project_with_documents):
        ...
        # 反馈通过迭代处理
        resp = client.post(f"/api/v1/projects/{project_with_documents}/iteration/run", json={"badcases": []})
        assert resp.status_code == 200
...
class TestUATEndToEndFullFlow:
    ...
    def test_full_delivery_lifecycle(self, client):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            ...
            tmp = f.name
        with open(tmp, "rb") as f:
            resp = client.post(f"/api/v1/projects/{pid}/documents", files={"file": ("test_doc.txt", f, "text/plain")})
        os.unlink(tmp)
        assert resp.status_code == 200

        # 3. 调研分析
        resp = client.post(f"/api/v1/projects/{pid}/research/run", json={"client_requirements": "测试需求"})
        assert resp.status_code == 200

        # 4. 方案设计
        resp = client.post(f"/api/v1/projects/{pid}/design/run", json={"badcases": []})
        assert resp.status_code == 200

        # 5. 生成Benchmark
        resp = client.post(f"/api/v1/projects/{pid}/benchmarks/generate", json={"case_count": 5})
        assert resp.status_code == 200
        ...
        # 7. 夜间迭代
        resp = client.post(f"/api/v1/projects/{pid}/iteration/run", json={"badcases": []})
        assert resp.status_code == 200
