...
def cmd_serve(args, client):
    """启动后端服务（开发模式）"""
    import subprocess

    port = args.port
    print(f"🚀 启动 AI-FDE Engine 服务: http://localhost:{port}/dashboard")
...
def cmd_doc_upload(args, client):
    """上传业务文档"""
    path = args.file
    ...
    result = client.request(
        "POST",
        f"/projects/{args.project_id}/documents",
        files={"file": (os.path.basename(path), content, "application/octet-stream")},
    )
    if result.get("success"):
        print(f"✅ 已上传: {os.path.basename(path)}")
    else:
        print(f"❌ 上传失败: {result.get('error', result)}")


def cmd_badcase_submit(args, client):
    """提交Badcase"""
    result = client.request(
        "POST",
        ...
        {
            "input": args.input,
            "actual_output": args.actual or "",
            "expected_output": args.expected or "",
    ...
    if result.get("success"):
        ...
        print(f"❌ 提交失败: {result.get('error', result)}")


def cmd_badcase_list(args, client):
    """列出Badcase"""
    ...
    if result.get("success"):
        for b in result.get("badcases", []):
            status = "🟢待处理" if b.get("status") == "open" else "✅已处理"
            print(f"  {status} [{b.get('error_type', '-')}] {(b.get('input') or '')[:60]}")
...
def cmd_export(args, client):
    ...
    item = args.item
    url = f"{client.base_url}/api/v1/projects/{args.project_id}/export"
    if item:
        from urllib.parse import quote

        url += f"?format=md&item={quote(item)}"
        out = args.output or f"{item}.md"
    else:
        fmt = args.format or "docx"
        url += f"?format={fmt}"
        out = args.output or f"aifde-deliverables.{fmt}.zip"
...
def cmd_templates(args, client):
    """行业模板列表"""
    result = client.request("GET", "/templates")
    if result.get("success"):
        print("📋 行业模板库：")
        for t in result.get("templates", []):
            print(f"  🏭 {t['industry']}（{t['key']}）: {t['name']} - {t['description']}")
            print(f"     需求模板 {t['requirement_count']} 条 / Benchmark种子 {t['benchmark_seed_count']} 条")


def cmd_golden(args, client):
    """黄金集质量评估（需配置真实LLM Key）"""
    from src.evaluation.golden import main as golden_main

    golden_main()


def cmd_llm_config(args, client):
    """配置LLM Key（热生效）"""
    if args.show:
        result = client.request("GET", "/settings/llm")
        llm = result.get("llm", {})
        print(f"Provider: {llm.get('provider')} | 模式: {llm.get('mode')} | Key: {llm.get('api_key_masked') or '-'}")
        return
    if not args.api_key:
        print("❌ 请提供 --api-key")
        sys.exit(1)
    result = client.request(
        "POST",
        "/settings/llm",
        {"provider": args.provider, "api_key": args.api_key, "base_url": args.base_url},
    )
    if result.get("success"):
        print(f"✅ 配置已热生效（{result['llm']['mode']} 模式）")
    else:
...
环境变量:
  ...
  AIFDE_API_KEY    API密钥 (可选)
    ...
    agent_p.add_argument(
        "agent_type", choices=["research", "design", "delivery", "benchmark", "iteration"], help="Agent类型"
    )
    ...
    # ===== v0.1.4 新增 =====
    # serve
    serve_p = subparsers.add_parser("serve", help="启动后端服务")
    serve_p.add_argument("--port", type=int, default=8000, help="端口")
    serve_p.add_argument("--reload", action="store_true", help="热重载（开发）")

    # doc
    doc_p = subparsers.add_parser("doc", help="文档管理")
    doc_sub = doc_p.add_subparsers(dest="doc_command")
    up_d = doc_sub.add_parser("upload", help="上传业务文档")
    up_d.add_argument("--project-id", required=True, help="项目ID")
    up_d.add_argument("--file", required=True, help="文件路径")

    # badcase
    bc_p = subparsers.add_parser("badcase", help="Badcase管理")
    bc_sub = bc_p.add_subparsers(dest="badcase_command")
    sub_bc = bc_sub.add_parser("submit", help="提交Badcase")
    sub_bc.add_argument("--project-id", required=True)
    sub_bc.add_argument("--input", required=True, help="用户输入")
    sub_bc.add_argument("--actual", help="实际错误输出")
    sub_bc.add_argument("--expected", help="期望输出")
    sub_bc.add_argument("--type", default="knowledge", choices=["knowledge", "prompt", "rule", "model_boundary"])
    sub_bc.add_argument("--severity", default="medium", choices=["low", "medium", "high", "critical"])
    list_bc = bc_sub.add_parser("list", help="列出Badcase")
    list_bc.add_argument("--project-id", required=True)
    list_bc.add_argument("--status", choices=["open", "processed"])

    # export
    exp_p = subparsers.add_parser("export", help="导出交付物")
    exp_p.add_argument("--project-id", required=True)
    exp_p.add_argument("--format", choices=["md", "docx"], default="docx", help="zip内文件格式")
    exp_p.add_argument("--item", help="单项key（导出markdown，如 02-需求基线）")
    exp_p.add_argument("--output", "-o", help="输出文件路径")

    # templates / golden / settings
    subparsers.add_parser("templates", help="行业模板列表")
    subparsers.add_parser("golden", help="黄金集LLM质量评估（需真实Key）")
    llm_p = subparsers.add_parser("llm", help="LLM配置")
    llm_p.add_argument("--show", action="store_true", help="查看当前配置")
    llm_p.add_argument("--provider", default="deepseek", choices=["deepseek", "qwen"])
    llm_p.add_argument("--api-key", help="API Key")
    llm_p.add_argument("--base-url", help="自定义Base URL")
...
def main():
    ...
    try:
        if args.command == "health":
            ...
            cmd_serve(args, client)
        elif args.command == "doc":
            if args.doc_command == "upload":
                cmd_doc_upload(args, client)
            else:
                parser.parse_args(["doc", "--help"])
        elif args.command == "badcase":
            if args.badcase_command == "submit":
                cmd_badcase_submit(args, client)
            elif args.badcase_command == "list":
                cmd_badcase_list(args, client)
            else:
                parser.parse_args(["badcase", "--help"])
        elif args.command == "export":
            cmd_export(args, client)
        elif args.command == "templates":
            cmd_templates(args, client)
        elif args.command == "golden":
            cmd_golden(args, client)
        elif args.command == "llm":
            cmd_llm_config(args, client)
        else:
            parser.print_help()
